...
// Generic secret storage for small values (e.g. the crypto-store key record):
// OS keychain on desktop, localStorage on web. Caller owns the full key name.
...
export async function secretDelete(key: string): Promise<void> {
  if (isTauri()) {
    try {
      await tauriInvoke("secret_delete", { key });
    } catch {
